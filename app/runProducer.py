...
class Produce:
    ...
    def __init__(self, doLoop, numberLoops, users, minLatency, maxLatency, bootstrap, topic, print_nth_msg):
        self.name = "Produce"
        self.doLoop = doLoop
    ...
    def generateAndPublish(self, topic) :
        keyVal, jsonObj = gameplay.generateGamePlay(self.users)
...
def main():
    ...
    parser.add_argument('-u', '--users', help='Max number of players. Defaults to 10.', required=False, metavar="")

    args = parser.parse_args()
    topic = args.topic
    doLoop = args.loop
    users = 10 if args.users is None else args.users
    ...
    p = Produce(doLoop, numberLoops, users, minLatency, maxLatency, bootstrap, topic, nthMsg)
